...
class AccountServiceTest : BehaviorSpec() {
    init {
        ...
        Given("이메일을 통해 유저를 검색하는데") {
            ...
            When("존재하는 유저라면") {
                val id = "1234"
                every { tUserRepository.findByEmail(any()) } answers { TUser.generateTmpModel().copy(id = id) }
                val result = service.findByEmail("do.judo1224@gmail.com")
                Then("해당 유저 객체를 반환한다") {
        ...
        Given("유저 등록 요청이 들어왔을 때") {
            When("등록된 유저의 이메일과 중복된다면") {
                every { tUserRepository.findByEmail(any()) } answers { TUser.generateTmpModel() }
