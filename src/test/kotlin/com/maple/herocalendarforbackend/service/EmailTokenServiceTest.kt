        ...
        Given("이메일 링크를 통해 인증하려는데") {
            When("유효하지 않은 토큰이라면") {
                ...
                Then("INVALID_AUTH_TOKEN 예외 발생") {
                    exception.errorCode shouldBe BaseResponseCode.INVALID_TOKEN
                }
            }
            ...
            When("유효한 토큰이고 등록된 인증정보가 있다면") {
                ...
                every { tEmailTokenRepository.delete(capture(tEmailTokenSlot)) } just Runs
                service.verifyEmail("")
                Then("유저 인증 정보를 갱신하고 이메일 토큰 데이터를 삭제") {
